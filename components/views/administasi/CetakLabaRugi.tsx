...
export default function CetakLabaRugi({ data }: { data: DataDataPengajuan[] }) {
  ...
  const handleExcelExport = () => {
    ...
    try {
      ...
      const newData: any[] = data.map((d: DataDataPengajuan, ind: number) => {
        ...
        const ciuPremi = d.DataPembiayaan.plafond * ((3 * inda) / 100);
        const resultPremi =
          d.jenis_asuransi === "BERDIKARI" ? premiAsuransi : ciuPremi;
        const selisihAsuransi = asuransi - resultPremi;
        const pencairan =
          plafond -
          (admin +
            adminBank +
        ...
        totalPremiAsuransi += resultPremi;
  ...
  return (
    <div className="flex justify-center">
      ...
      >
        Cetak Laba Rugi {loading ? <LoadingOutlined /> : <PrinterOutlined />}
      </button>
    </div>
  );
}
