...
export default function TuggakanReguler() {
  ...
  const getData = async () => {
    ...
    const res = await fetch(
      `/api/angsuran/tunggakan-reguler?page=${page}${
        name ? "&name=" + name : ""
      }`
    );
  ...
  const handleClick = async (id: string) => {
    setLoading(true);
    const res = await fetch(`/api/angsuran/reguler-antri`, {
      method: "PUT",
  ...
  return (
    <div>
      ...
      <div>
        <Table
          columns={columns}
          dataSource={data?.sort((a, b) => a.angsuran_ke - b.angsuran_ke)}
          bordered
          size="small"
          loading={loading}
          pagination={{
