...
const InputPelunasan = dynamic(
  () => import("@/components/views/repayment/InputPelunasan"),
  {
    ssr: false,
    loading: () => <LoadingOutlined />,
  }
...
export default function Pelunasan() {
  ...
  const [dataPengajuan, setDataPengajuan] = useState<DataDataPengajuan[]>();

  const getData = async () => {
    setLoading(true);
    const res = await fetch(
      `/api/repayment?page=${page}&pageSize=${pageSize}${
        name ? "&name=" + name : ""
      }${month ? "&month=" + month : ""}`
    );
    const { data, total, dataPengajuan } = await res.json();
    setData(data);
    setTotal(total);
