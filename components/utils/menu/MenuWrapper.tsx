...
export default function MenuWrapper({ user }: { user: any }) {
  ...
  return (
    <div style={{ width: collapse ? 100 : 280 }} className="hidden md:block">
      <div className={`flex flex-col gap-2`}>
        <div
          className={`flex gap-4 text-xs items-center bg-${
            process.env.NEXT_PUBLIC_APP_BG_BIO
          ...
          <div>
            ...
            <div style={{ marginTop: 5 }}>
              {user?.role === "BANK" || user?.role === "APPROVAL" ? (
                <span>
                  {user?.Bank.kode} |{" "}
                  <span className="text-xs">{user?.posisi}</span>
                </span>
              ) : (
                <span>
        ...
        <div style={{ flex: 1 }}>
          <div
            className={`bg-white p-1 flex ${
              collapse ? "justify-center" : "justify-end"
            }`}
          >
            <button
              className="border px-2 py-0 rounded shadow"
