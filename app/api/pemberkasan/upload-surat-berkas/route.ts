...
export const GET = async (req: NextRequest) => {
  ...
  let result: DataPenyerahanBerkas[] = [];
  if (name) {
    result = <any>await prisma.penyerahanBerkas.findMany({
      where: {
        nomor_surat_berkas: { contains: name },
        is_active: true,
      },
      include: {
        Bank: true,
...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    const fileName = `${data.kode_area}_SURAT-BERKAS_${moment().format(
      "DDMMYYYY"
    )}.${data.ext}`;
...
export const PUT = async (req: NextRequest) => {
  const data = await req.json();
  try {
    await prisma.penyerahanBerkas.update({
      where: {
        id: data.id,
      },
      data: {
        surat_berkas: data.surat_berkas,
        tanggal_surat_berkas: data.tanggal_surat_berkas,
      },
    });
    return NextResponse.json(
      {
